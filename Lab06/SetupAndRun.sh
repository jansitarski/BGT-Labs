...
pv db.tar | sudo -u postgres pg_restore -d music

for i in {1..5}; do
  sudo -u postgres psql -d music -c '\timing' -c '
  SELECT tr.name AS "Track Name", re.name AS "Album Name", art.name AS "Artist Name", ar1.name AS "Artist Country", ar2.name AS "Release Country"
    ...
    JOIN (SELECT area.id, area.name FROM musicbrainz.area) ar2 ON rel_c.country=ar2.id
  limit 1000000000;' | tail -1 >> results.txt
done
